...
在本部分中，你将创建一个 PayaraMicro 应用程序并在本地测试它。
...
在本部分中，你将配置 PayaraMicro 项目的 pom.xml 文件，以便 Maven 可以将应用部署到 Linux 上的 Azure 应用服务。
...
4. 将运行时从 `TOMCAT 8.5` 修改为 `java11`将部署文件从 `*.war` 修改为 `*.jar`。 然后，将 `<appSettings>` 部分添加到 `PORT`、`WEBSITES_PORT` 和 `WEBSITES_CONTAINER_START_TIME_LIMIT` 的 `<configuration>` 部分。  
