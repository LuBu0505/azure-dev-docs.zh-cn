...
## <a name="configure-chef-server"></a>配置 Chef 服务器
...
### <a name="create-a-hosted-chef-account"></a>创建托管 Chef 帐户

在[此处](https://manage.chef.io/signup)注册托管 Chef 帐户。

在注册过程中，系统会要求创建新的组织。

![“创建组织”窗口](media/windows-vm-configure/create-organization.png)

在创建组织后，下载初学者工具包。

![配置 Chef](media/windows-vm-configure/configure-chef.png)
...
### <a name="configure-a-chef-workstation"></a>配置 Chef 工作站
...
![Knife 文件示例](./media/windows-vm-configure/knife-file-example.png)
...
## <a name="create-a-chef-cookbook"></a>创建 Chef 食谱
...
## <a name="upload-the-chef-cookbook-to-the-chef-server"></a>将 Chef 食谱上传到 Chef 服务器
...
![正在预配虚拟机](./media/windows-vm-configure/virtual-machine-being-provisioned.png)

接下来显示命令提示符。

![创建虚拟机时的 Knife 输出](./media/windows-vm-configure/knife-output-when-creating-vm.png)

部署完成后，将显示新虚拟机的公共 IP 地址。 请将此值粘贴到 Web 浏览器中，以查看新网站。 部署虚拟机时，我们已打开端口 80，使其可在外部使用。

![测试虚拟机](./media/windows-vm-configure/vm-test.png)

此示例使用了富有创造性的 HTML 代码。

还可以在 [Chef 管理](https://manage.chef.io/)中查看节点的状态。

![查看节点状态](./media/windows-vm-configure/view-node-status.png)
